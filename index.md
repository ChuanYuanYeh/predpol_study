...
On the other hand, arrest numbers in all divisions except for Hollywood and Central saw gradual decreases according to the animated map below. Our guess was that perhaps not all types of crimes were affected the same by PredPol. For example, increasing police presence in an area might discourage people to commit crimes that can be easily spotted out in public. However, crimes that are more subtle such as computer hacking would not be affected as much. Thus, we then looked at how the distribution of crime types changed after PredPol was deployed.
![Annual Arrests in LA](/assets/arrests_animated.gif)

The two maps below shows the simplified version of results from statistically testing the differences between the proportion of different crime types before and after Pred1Pol's deployment. Our null hypothesis was that the proportion of a specific crime type, say personal crimes, was the same after PredPol was deployed. In the maps, values of 1 implies that there was a significant increase of a crime type in a division, and values of -1 means otherwise. Values of 0 means that either there were no differences or they were not significant at a significance level of 0.05.
